...
- **最近添加记录**：在扩展弹窗中快速查看最近通过 AnkiConnect 或离线队列添加的 5 条卡片记录，支持一键删除。
...
- **Anki 离线队列**：新增自动化降级与离线存储机制。当 AnkiConnect 网络不可达或手动设为 `none` 导出模式时，制卡请求将自动进入本地队列。支持批量上传恢复、单条精准删除以及基于 JSON 的数据导入导出备份，在 `none` 模式下可直接作为本地离线采集。
- **Bug 修复**：
